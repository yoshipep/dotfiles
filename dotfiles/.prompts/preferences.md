...
- Variables must be declared at the beginning of the functions. First place declared but uninitialized variables, then
   ...
   ```
- After an if, switch or loop statement, leave an empty line. There are some cases where no empty line must be written: if the statement is the last one in a block; If the statement is the last one in the function; If the statement is just before the return value. As an example look at the following snippet:
   ```c
       for (i = 0; i < 100; ++i) {
           if (cond1)
               nCoindicendes++;
...
- Header files must use `.hpp` extension unless this header is intended to also be used in C. If that's the case and you
    build the file, then add appropriate guards (`#ifdef __cplusplus`).
